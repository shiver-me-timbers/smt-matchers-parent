...
public class AfterWithinDateMatcherTest {
    ...
    public void Can_check_that_a_date_is_within_a_range_after_another_date() {
        ...
        given(date2.after(date1)).willReturn(true);
        given(timeOperations.isBeforeOrEqualTo(dateTime2, dateTime1 + durationInMillis)).willReturn(true);

        // When
        final boolean actual = new AfterWithinDateMatcher(timeOperations, date1, duration, unit).matches(date2);
    ...
    public void Can_check_that_a_date_is_before_a_range_after_another_date() {
        ...
        given(date1.getTime()).willReturn(dateTime1);
        given(date2.getTime()).willReturn(dateTime2);
        given(date2.after(date1)).willReturn(false);
        given(timeOperations.isBeforeOrEqualTo(dateTime2, dateTime1 + durationInMillis)).willReturn(true);
    ...
    public void Can_check_that_a_date_is_after_a_range_after_another_date() {
        ...
        given(date2.getTime()).willReturn(dateTime2);
        given(date2.after(date1)).willReturn(true);
        given(timeOperations.isBeforeOrEqualTo(dateTime2, dateTime1 + durationInMillis)).willReturn(false);
